   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
    ...
    XeonBotInc.ev.on('messages.upsert', async chatUpdate => {
        ...
        try {
            ...
            require("./kawaki-bug-bot")(XeonBotInc, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
...
XeonBotInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
			...
			await delay(1999)
            console.log(chalk.yellow(`\n\n                  ${chalk.bold.blue(`𝙺𝙰𝚆𝙰𝙺𝙸-𝙱𝚄𝙶`)}\n\n`))
            console.log(chalk.cyan(`< =✰======================✰=0-𝚂𝙾𝚄𝙲𝙸𝚂=✰=============✰= >`))
	        console.log(chalk.magenta(`\n${themeemoji} YT CHANNEL: Jean-parker-tech`))
            console.log(chalk.magenta(`${themeemoji} GITHUB: kawaki227`))
            console.log(chalk.magenta(`${themeemoji} INSTAGRAM: @its_kawaki2279 `))
